...
static void next_inst(struct cpu_ctx *r)
{
...
#ifdef TRACE_6502
    printf("%02x %02x %02x %02x: ", r->a, r->x, r->y, r->sp);
#endif
...
#ifdef TRACE_6502
        printf("lda/sta to IO: op $%02x %s\n", op_code, info->op->fmt);
#endif
        return;
    }

#ifdef TRACE_6502
    printf("%04x %s", oldpc, info->op->fmt);
    ...
    if(info->mode->fmt != NULL)
    {
        ...
        {
            printf("$%04x", r->pc + arg->rel.value);
        }
        else
        {
            printf(info->mode->fmt, value);
        }
        switch (mode)
        {
        case MODE_RELATIVE:
